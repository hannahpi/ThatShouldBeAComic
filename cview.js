...
//Used to check if image is loaded:
  // found solution at http://stackoverflow.com/questions/1977871/check-if-an-image-is-loaded-no-errors-in-javascript
  // which references also: http://www.sajithmr.me/javascript-check-an-image-is-loaded-or-not/
...
function prepareImage()
{
	...
	if (!(isImgLoaded(galleryarray[curimg]))) {
		$(galleryarray[curimg]).load(function() { prepareImage(); });
		...
		cview.drawImage(loadingImg, 0, 0,424,283);
		// code to draw loading image.
		return;
	}
	...
	drawRotate(degrees);
	//removing the number of load checks behavior
...
function setsize()
{
	...
	doData("setResolution.php?max=" + 1024);
}
